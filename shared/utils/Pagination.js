...
class Pagination {
  ...
  async getEdges(queryArgs) {
    ...
    if (!first && !last) {
      ...
      throw new ApolloServerErrorCode.BAD_USER_INPUT("Search queries may only be paginated forward.");
    }
  ...
  async _getFilterWithCursor(fromCursorId, filter, operator, sort) {
    ...
    if (!fromDoc) {
      throw new ApolloServerErrorCode.BAD_USER_INPUT(`No record found for ID '${fromCursorId}'`);
    }
